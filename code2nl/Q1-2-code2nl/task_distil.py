...
from distilModel import TeacherSeq2Seq,StudentSeq2Seq
...
def distill_loss(predicts, targets, temperature=1.0):
    ...
    ans=- targets_prob * student_likelihood
    # 将ans按最后一维求和
    ans=ans.sum(dim=-1)
    ans2=ans.mean()
    return ans2
...
def main():
    ...
    # 通过torch保存的state_dict() 加载 teacher_model
    ...
    teacher_model.load_state_dict(torch.load('/home/wuruifeng/data/result/code2nl/Finetune/checkpoint-best-bleu/pytorch_model.bin'))
    # build model
    ...
    if args.do_train:
        ...
        train_dataloader = DataLoader(train_data, sampler=train_sampler,
                                      batch_size=args.train_batch_size // args.gradient_accumulation_steps,num_workers=8)
            ...
            if args.do_eval and ((global_step + 1) % args.eval_steps == 0) and eval_flag:
                if args.hid_distill:
                    if (global_step+1) %5000==0:
                        temp=os.path.join(args.output_dir, str(global_step+1)+"_hid")
                        output_dir = os.path.join(temp, 'checkpoint-best')
                ...
                for batch in eval_dataloader:
                    ...
                    with torch.no_grad():
                        _, loss, num = student_model(source_ids=source_ids, source_mask=source_mask,
                ...
                for batch in eval_dataloader:
                    ...
                    with torch.no_grad():
                        preds = student_model(source_ids=source_ids, source_mask=source_mask)
                        for pred in preds:
                            t = pred[0].cpu().numpy()
                            t = list(t)
                            if 0 in t:
        ...
        # 输出最优结果
        if args.pred_distill or args.tradition_distill:
            logger.info("  Best bleu:%s", best_bleu)
            logger.info("  Best ppl:%s", np.exp(best_loss))
    ...
    if args.do_test:
        ...
        for idx, file in enumerate(files):
            ...
            for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                ...
                source_ids, source_mask = batch
                with torch.no_grad():
                    preds = studnet_model(source_ids=source_ids, source_mask=source_mask)
                    for pred in preds:
                        t = pred[0].cpu().numpy()
                        t = list(t)
                        if 0 in t:
