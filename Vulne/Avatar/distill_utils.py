...
def train(student_model, teacher_model, map_function, train_dataloader, eval_dataloader, hid_epoches, pred_epoches,
          hid_learning_rate, pred_learning_rate, temperature, device, loss_function, hid_distil,surrogate=False):
    # hid_distil
    ...
    for epoch in range(pred_epoches):
        ...
        for batch in bar:
            ...
            cls_loss = distill_loss(student_logits, soft_labels, temperature=temperature)
...
def distill(args,hyperparams_set,surrogate=False):
    ...
    for hyperparams in hyperparams_set:
        ...
        # 准备训练数据
        train_dataset = TextDataset(tokenizer_type, vocab_size, args.train_data_file, max_sequence_length)
        train_sampler = RandomSampler(train_dataset)
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=batch_size)

        eval_dataset = TextDataset(tokenizer_type, vocab_size,  args.eval_data_file, max_sequence_length)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_data_file", default="/home/wuruifeng/data/data/Vulnerability-Detection/data/soft_label_train_codebert.jsonl", type=str, required=False,
    ...
    parser.add_argument("--pred_epoches", type=int, default=8,
    ...
    args.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")  # 多GPU时可指定起始位置/编号
